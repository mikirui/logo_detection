...
def Score(svm,paras):
	...
	for i in range(0,8):             #seperate data
		...
		for j in range(0,val_x.shape[0]):
			...
			if(pred == val_label[j]):
				cnt += 1
		print("%dth batch: predict correct %d times" % (i,cnt))
...
for C in C_grid:
	for gamma in gamma_grid:
		...
		svm = cv2.SVM()
		s = Score(svm,svm_params)
		print("C = %f ,gamma = %f, score = %f" % (C,gamma,s))
		if(s>max_score):
			max_score = s
			best_C = C
			best_g = gamma

print("Best perfomance:\nC = %f ,gamma = %f, score = %f" % (best_C,best_g,max_score))
'''
for C in C_grid:
	svm_params = dict(kernel_type = cv2.SVM_LINEAR,svm_type = cv2.SVM_C_SVC,
		C=C)
	svm = cv2.SVM()
	s = Score(svm,svm_params)
	print("C = %f , score = %f" % (C,s))
	if(s>max_score):
		max_score = s
		best_C = C

print("Best perfomance:\nC = %f, score = %f" % (best_C,max_score))
'''